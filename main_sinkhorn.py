...
def train():
    ...
    with tf.Session(config=config) as sess:
        ...
        if FLAGS.train_stage == 'ranker':
            if FLAGS.use_debias:
                check_point_dir = FLAGS.train_dir + FLAGS.train_stage + "/sinkhorn/debias_"+ str(
                    FLAGS.hparams) + '_batch_'+ str(FLAGS.batch_size)
            else:
                check_point_dir = FLAGS.train_dir + FLAGS.train_stage + "/sinkhorn/withoutdebias_" + str(
                    FLAGS.hparams) + '_batch_' + str(FLAGS.batch_size)
        ...
        if FLAGS.train_stage == 'click':
            if FLAGS.use_debias:
                check_point_dir = FLAGS.train_dir + FLAGS.train_stage +"/debias_clip_" + str(
                    FLAGS.hparams) + '_batch_' + str(FLAGS.batch_size)
            else:
            ...
            while True:
                ...
                if current_step % FLAGS.steps_per_checkpoint == 0:
                    ...
                    while it < valid_set.item_num / model.batch_size:
                        ...
                        v_loss, summary, acc, l2_loss = model.bias_step(sess, input_feed, forward_only=True)
                        it += model.batch_size
                        valid_loss += v_loss
                        count_batch += 1.0
                        acc_sum += acc
        ...
        if FLAGS.train_stage == 'ranker':
            for i in range(1):
                ...
                for it in tqdm(range(int(test_set.user_num))):
                    ...
                    labels, scores = model.ranker_step(sess, input_feed, labels, clicks, forward_only=True)
                    # test KM oracle
                    # labels, KM_scores, click_table = model.KM_oracle(sess, input_feed, labels, clicks, True, click_model_1)
                    # test KM ranker
            ...
            logloss = log_loss(label_ary, click_ary)

            rel_error = rel_error / count_batch
            print("  test: loss %.2f auc %.4f acc %.4f rel_err %.4f log_loss %.4f" % (valid_loss, auc, acc, rel_error, logloss))
