    ...
    def __init__(self, pred_scores, rel_list, dataset):
        ...
        for i in range(self.query_num):
            ...
            if len(pos) != len(dataset.propensity[i]):
                print('ERROR! position do not match!', i, pos, dataset.propensity[i])
                exit()
        ...
        self.click_prob_list = []
        for ii in range(dataset.rank_list_size):
            if self.click_prob_num_list[ii]  == 0:
                self.click_prob_list.append(self.click_prob_sum_list[ii])
            else:
                self.click_prob_list.append(self.click_prob_sum_list[ii] / self.click_prob_num_list[ii])
...
def compute_metrics(labels, scores, dataset, save_file=None, log=None):
    ...
    for i in [1, 3, 5, 10]:
        NDCG = metrics.NDCG(i)
        ERR = metrics.EER(i)
        print('NDCG@' + str(i) + ': ', NDCG, end=' ')
        print('EER@' + str(i) + ': ', ERR)
        if log:
            print("NDCG@ %d %.6f  ERR@ %d %.6f" % (i, NDCG, i, ERR), file=log, flush=True)
    print()
