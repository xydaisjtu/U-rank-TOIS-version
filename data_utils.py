...
class Raw_data:
    def __init__(self, data_path=None, set_name=None, rank_cut=100000):
        ...
        self.feature_path = data_path + 'origin/'
        ...
        for ranker_name in ['ranker1', 'ranker2']:
            ...
            #     self.bids.append([float(x) for x in arr])
            ...
            for line in gold_weight_fin:
                ...
                flatten_rls.extend(self.relavance_labels[-1])
            ...
            for line in click_fin:
                ...
                flatten_clicks.extend(self.clicks[-1])
            ...
            for line in exam_fin:
                ...
                flatten_prop.extend(self.propensity[-1])
            exam_fin.close()
            print(ranker_name, 'exams ', len(flatten_prop))
            # if exam_type == 'true':
            #     self.esti_prop = self.propensity
            # if exam_type == 'rand':
            #     exam_fin = open(self.data_path + ranker_name + '/' + set_name + '/' + 'RandomizedPropensityEsitimateResult.txt')
            #     exam = json.load(exam_fin)["IPW_list"]
            #     exam = [1/x for x in exam]
            #     while len(self.esti_prop) < len(self.len_list):
            #         p = exam[:self.len_list[len(self.esti_prop)]]
            #         self.esti_prop.append(p)
        ...
        #         self.esti_prop.append([1 for _ in range(self.len_list[i])])
        #
        # if exam_type == 'CPBM':
        #     exam_fin = open('../CPBM/data/CPBM/'+ data_dir_name +'/SVMproprank/CPBM_' + set_name + '.exam', 'r')
        #     for i, line in enumerate(exam_fin):
        #         exam_line = [float(x) for x in line.strip().split(' ')][:self.len_list[i]]
        #         self.esti_prop.append(exam_line)
        #     print("CPBM", len(self.esti_prop) == len(self.propensity))

        print('did', len(flatten_dids))
        print('qid', len(flatten_qids))
        print('prop', len(flatten_prop))

        self.flatten_dids = flatten_dids
        self.flatten_poss = flatten_poss
        self.doc_features = np.array(self.doc_features)

        self.data = np.ones([len(flatten_dids), 6], dtype=np.float)  # (qid, did, pos, exam, click)
...
def test():
    ...
    for i in range(20):
        ...
        # print(model.context_features[i])
        # print(model.pos_clicks[i])
        # print(model.neg_clicks[i])
        print(' ')
